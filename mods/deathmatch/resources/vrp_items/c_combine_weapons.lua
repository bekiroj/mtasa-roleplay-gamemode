...
function traceBullet(weapon, ammo, ammoInClip, hitX, hitY, hitZ, hitElement, startX, startY, startZ )
	triggerServerEvent("items.useAmmo", getLocalPlayer(), weapon, 1, hitX, hitY, hitZ, hitElement, startX, startY, startZ )
end
...
function weaponServerSync()
	...
	if tonumber(getElementData(localPlayer,"paintball.areaid")) then return end
	if (loggedin==1) then
		local weaponArr = { }
		for i=0, 12 do
