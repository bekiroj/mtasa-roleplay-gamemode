...
local function updateWorldItemValue(item, station, volume)
    if station < 0 or station > #exports.vrp_carradio:getStreams() then return end
    ...
    exports.vrp_anticheat:changeProtectedElementDataEx(item, "itemValue", newvalue)
    dbExec(exports.vrp_mysql:getConnection(), "UPDATE worlditems SET itemvalue='" .. newvalue .. "' WHERE id=" .. getElementData(item, "id"))
    triggerClientEvent("toggleSound", item)
end

function changeTrack(item, step)
    local streams = exports.vrp_carradio:getStreams()
	...
	if not tuned[item] then
		exports.vrp_global:sendLocalMeAction(source, "retunes the Ghettoblaster.")
		tuned[item] = true
	else
