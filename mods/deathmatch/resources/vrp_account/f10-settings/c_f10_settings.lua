...
function showSettingsWindow(tab)
	...
	if getResourceFromName("vrp_carradio") then
	...
	if getResourceFromName("vrp_logs") then
	...
	if getResourceFromName("vrp_logs") and getResourceFromName("vrp_phone") then
	...
	if getResourceFromName("vrp_chat") then
	...
	if getResourceFromName("vrp_chat") and guiCheckBoxGetSelected(GUIEditor_Checkbox.graphic_chatbub) then
	...
	if getResourceFromName("vrp_hud") then
	...
	if getResourceFromName("vrp_shader_radar") then
	...
	if getResourceFromName("vrp_shader_water") then
	...
	if getResourceFromName("vrp_shader_car_paint") then
	...
	if getResourceFromName('vrp_vehicle') then
	...
	if getResourceFromName("vrp_realism") then
	...
	posY = posY + lineH
	local dyn_light = exports.vrp_global:isResourceRunning('vrp_dynamic_lighting') and true or false
	...
	local xmas = exports.vrp_global:isResourceRunning('vrp_xmas') and true or false
	if xmas then
		posY = posY + lineH
	...
	if exports.vrp_integration:isPlayerSupporter(getLocalPlayer()) or exports.vrp_integration:isPlayerTrialAdmin(getLocalPlayer()) then
		posY = posY + lineH
	...
	if getResourceFromName("vrp_description") then
...
function applyGameSettings(name, value)
	if name and value then
		...
		elseif name == "hide_hud" then
			...
			elseif exports.vrp_global:hasItem(localPlayer, 111) then
				setPlayerHudComponentVisible("radar", true)
			end
		elseif name == "groundsnow" then
			local groundsnow = exports.vrp_global:isResourceRunning('vrp_shader_snow_ground') and true or false
			if groundsnow then
				if value == "0" then
