		...
		if gui.main and isElement(gui.main) then
			...
			addEventHandler("onClientGUIClick", gui.refresh, function()
				if source == gui.refresh then
					...
					exports.vrp_global:playSoundCreate()
		...
		function( button )
			if button == "left" then
				...
				if row ~= -1 and col ~= -1 then
					if exports.vrp_integration:isPlayerLeadAdmin(localPlayer) then
		...
		function( button )
			if button == "left" then
				...
				if row ~= -1 and col ~= -1 then
					...
					if owner == username or exports.vrp_integration:isPlayerLeadAdmin(localPlayer) then
...
function openNewStation(id, name, ip, state, order, buyNew)
	...
	exports.vrp_global:centerWindow(gui.wNewStation)
	local margin = 10
	local lineH = 25
	local col1 = 85
...
function renewStation(station)
	...
	exports.vrp_global:centerWindow(gui.wRenewStation)
	local margin = 20
	local lineH = 16
