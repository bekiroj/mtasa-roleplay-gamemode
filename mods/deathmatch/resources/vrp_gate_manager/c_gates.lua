...
function setGateClientSideStuff(element)
	setObjectBreakable(element, false)
end
addEvent("gates:setClientSideStuff", true)
addEventHandler("gates:setClientSideStuff", resourceRoot, setGateClientSideStuff)

function clientGetGates(gates)
	if gates then
		if #gates > 0 then
			for k,v in ipairs(gates) do
				setGateClientSideStuff(v)
			end
		end
	end
end
addEvent("gates:initialClientReceive", true)
addEventHandler("gates:initialClientReceive", resourceRoot, setGateClientSideStuff)