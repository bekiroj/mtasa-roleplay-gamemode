...
local freeCamLimits = false--{x1,y1,z1,x2,y2,z2}
local camint = false
local camdim = false
...
local function freecamFrame ()
    ...
    local int = getCameraInterior()
    if int ~= camint then
    	setCameraInterior(camint)
    end

    -- Set the new camera position and target
    if freeCamLimits and freeCamLimits[1] <= camPosX and freeCamLimits[4] >= camPosX and freeCamLimits[2] <= camPosY and freeCamLimits[5] >= camPosY and freeCamLimits[3] <= camPosZ and freeCamLimits[6] >= camPosZ then
...
function setFreecamEnabled (x, y, z, limits, int,dim)
	if isFreecamEnabled() then
		return false
	end
	if limits then
		freeCamLimits = limits
	end
	if int then
		camint = int
		setCameraInterior(int)
		local curint = getElementInterior(localPlayer)
		setElementData(localPlayer, "freecam:oldint", curint)
	end
	--outputChatBox(toJSON(limits))
...
function setFreecamDisabled()
	...
	local int = getElementData(localPlayer, "freecam:oldint") or false
	local dim = getElementData(localPlayer, "freecam:olddim") or false

	if int then
		setCameraInterior(int)
		--outputChatBox("inter"..int)
		setElementData(localPlayer,"freecam:oldint",false)
	end
	--[[
	if dim then
		setElementDimension(getCamera(  ),dim)
		setElementData(localPlayer,"freecam:olddim",false)
	end-]]
