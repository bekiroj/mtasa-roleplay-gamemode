...
function createComputerGUI(computerName, websiteToStartOn)
	...
	wComputer = guiCreateWindow(computerX, computerY, computerWidth, computerHeight + 29, computerName or "Bilgisayar", false)
	...
	if key == "left" and state == "down" then
		if(source == chatButton) then
			if (guiGetInputEnabled( )) then
				...
				showCursor(false)
				outputChatBox("Chatbox Aktif", 0, 255, 0, true)
				outputChatBox("'M' Tuşuna basarak bilgisayarı kullanmaya devam edebilirsiniz.")
			else
				guiSetInputEnabled(true)
				showCursor(false)
				outputChatBox("Bilgisayar ekranı aktif", 0, 255, 0, true)
			end
		end
	end
end
