...
function createElevator(thePlayer, commandName, oneway)
	if (exports.vrp_integration:isPlayerTrialAdmin(thePlayer) or exports.vrp_integration:isPlayerScripter(thePlayer) or exports.vrp_integration:isPlayerSupporter(thePlayer)) then
		...
		if id then
			...
			if (query) then
				...
				exports.vrp_global:giveItem(thePlayer, 73, id)
...
function createElevatorWithFriend(thePlayer, commandName, targetPlayer)
	if (exports.vrp_integration:isPlayerTrialAdmin(thePlayer) or exports.vrp_integration:isPlayerSupporter(thePlayer)) then
		if not (targetPlayer) then
			...
			local targetPlayer, tragetPlayerName = exports.vrp_global:findPlayerByPartialNick(thePlayer, targetPlayer)
			local x, y, z = getElementPosition(thePlayer)
			local interiorwithin = getElementInterior(thePlayer)
			local dimensionwithin = getElementDimension(thePlayer)
	...
	thread:query("SELECT rot, tprot, id, x, y, z, tpx, tpy, tpz, dimensionwithin, interiorwithin, dimension, interior, car, disabled, oneway FROM `elevators` WHERE id = " .. elevatorID,
		function(res, rows, err)
			for index, row in ipairs(res) do
				...
				exports.vrp_pool:allocateElement(elevatorElement, tonumber(row.id), true)
			end
		end
	)
	return true
...
function enterElevator(goingin)
	...
	if ( ( vehicle and elevatorStatus[INTERIOR_TYPE]  ~= 0 and getVehicleOccupant( vehicle ) == player ) or not vehicle ) then
		...
		local dbid, entrance, exit, interiorType, interiorElement  = call( getResourceFromName( "vrp_interiors" ), "findProperty", player, otherCP[INTERIOR_DIM] )
		if dbid > 0 then

		else
			dbid, entrance, exit, interiorType, interiorElement  = call( getResourceFromName( "vrp_interiors" ), "findProperty", player, currentCP[INTERIOR_DIM] )
		end
		...
		if vehicle then
			...
			if interiorElement and isElement(interiorElement) and getElementType(interiorElement) == "interior" then
				exports.vrp_anticheat:changeProtectedElementDataEx(interiorElement, "lastused", exports.vrp_datetime:now(), true)
				dbExec(mysql:getConnection(),"UPDATE interiors SET lastused=NOW() WHERE id="..dbid)
				exports.vrp_logs:dbLog("SYSTEM", 31, { interiorElement, player, vehicle } , "ENTERED/EXITED")
				exports["vrp_interior_manager"]:addInteriorLogs(dbid, "ENTERED/EXITED", player)
			end
		elseif isElement(player) then
			if movingInSameInt then
				...
				exports["vrp_interiors"]:setPlayerInsideInterior(interiorElement, player, otherCP,movingInSameInt)
			end
			return true
		else
...
function warpVehicleIntoInteriorfunction(vehicle, interior, dimension, offset, x,y,z,pickup,other, interiorElement,movingInSameInt)
	if isElement(vehicle) then
		...
		exports.vrp_anticheat:changeProtectedElementDataEx(vehicle, "health", getElementHealth(vehicle), false)
		...
		setTimer(function ()
			...
			exports.vrp_anticheat:changeProtectedElementDataEx(vehicle, "health")
...
function deleteElevator(thePlayer, commandName, id)
	if (exports.vrp_integration:isPlayerTrialAdmin(thePlayer) or exports.vrp_integration:isPlayerSupporter(thePlayer) or exports.vrp_integration:isPlayerScripter(thePlayer)) or commandName == "PROPERTYCLEANUP" then
...
function getNearbyElevators(thePlayer, commandName)
	if (exports.vrp_integration:isPlayerTrialAdmin(thePlayer) or exports.vrp_integration:isPlayerSupporter(thePlayer) or exports.vrp_integration:isPlayerScripter(thePlayer)) then
...
function delElevatorsFromInterior(thePlayer, commandName, intID)
	if (exports.vrp_integration:isPlayerTrialAdmin(thePlayer)) or commandName == "PROPERTYCLEANUP" then
		if (not tonumber(intID) or tonumber(intID)%1~= 0 ) and commandName ~= "PROPERTYCLEANUP" then
			...
			if tonumber(intID) == 0 and not exports.vrp_integration:isPlayerSeniorAdmin(thePlayer) then
...
addEventHandler( "toggleCarTeleportMode", getRootElement(),
	function( player )
		...
		if query then
			...
			exports.vrp_anticheat:changeProtectedElementDataEx( source, "status", elevatorStatus, false )
			if mode == 0 then
