...
function getPaperFromTony(button, state, absX, absY, wx, wy, wz, element)
    if (element) and (getElementType(element)=="ped") and (button=="right") and (state=="down") then --if it's a right-click on a object
        ...
        if(pedName == "Tony Johnston") then
            ...
            showCursor(true)
            local row = exports.vrp_rightclick:addRow("Buy DMV transaction paper - 100$")
            addEventHandler("onClientGUIClick", row,  function (button, state)
            	if exports.vrp_global:hasMoney(localPlayer, 100) and getElementData(localPlayer, "loggedin") == 1 then
...
function cBeginGUI()
	...
	greetingWindow = guiCreateStaticImage(x, y, width, height, ":vrp_resources/window_body.png", false)
...
function PlateWindow(vehicleList)
	...
	for i,r in ipairs(vehicleList) do
		...
		guiGridListSetItemText(vehlist, row, ov, exports.vrp_global:getVehicleName(v) , false, false) --tostring(getVehicleNameFromModel(vm))
...
function editPlateWindow()
	...
	if (source == vehlist) and not (svnum == "") then
		...
		efinalWindow = guiCreateStaticImage(x, y, width, height, ":vrp_resources/window_body.png", false)
...
function updateRegistration()
	...
	if (source == vehlist) and not (svnum == "") then
		...
		if exports.vrp_global:hasMoney(localPlayer,  state == "Unregistered" and 175 or 50 ) then
...
function insuranceDetails()
	...
	if (source == vehlist) and not (svnum == "") then
		...
		local insuranceFactionName = exports.vrp_factions:getFactionName(tonumber(data.insurancefaction))
		...
		string = string.."Premium: $"..exports.vrp_global:formatMoney(tonumber(data.premium)).."\n"
