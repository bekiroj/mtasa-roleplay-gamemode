...
addEventHandler("onClientResourceStart",getResourceRootElement(getThisResource()),function()
	...
	addCommandHandler("grab",function()
		if not getElementData(local_player,"hanging") then
			if target.vehicle and target.distance <= grab_distance then
				if isPedDead(local_player) or getElementHealth(local_player) <= 0.2 then
					return
				end
	...
	addEventHandler("onClientRender", getRootElement(), function()
		if not getElementData(local_player,"hanging") and getPedControlState('jump') and getPedSimplestTask(local_player) == 'TASK_SIMPLE_IN_AIR' then
