function openDutyWindow()
	...
	if #availablePackages > 0 then
		...
		outputChatBox("#575757Valhalla:#f9f9f9 Bu bölgede iş başı yapamazsın.", 255, 0, 0, true)
	end
end
...
function selectPackageGUI_open(availablePackages, allowList)
	...
	for pIndex, packageDetails in ipairs(availablePackages) do
		...
		for index, itemDetails in pairs(packageDetails[5]) do
			...
			gui[guiPrefix.."pushButton1"] = guiCreateButton(xAxis, yAxis, 71, 51, exports['vrp_items']:getItemName(itemDetails[2]), false, gui[guiTabName])
		...
		for skinIndex, skinID in pairs(skinTable) do
			...
			gui[guiTabName.."-skin-"..skinID] = guiCreateStaticImage (xAxis, yAxis, 75, 75, ":vrp_account/img/" .. skinImg..".png", false, gui[guiTabName] )
